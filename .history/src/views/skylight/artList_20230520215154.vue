    ...
    <el-card class="box-card">
        ...
        <div class="buttonBox">
          <router-link to="/home">
            <el-tooltip content="退出" placement="top">
              <el-button type="primary" icon="el-icon-s-home">退出</el-button>
            </el-tooltip>
          </router-link>
        </div>
      </div>
        ...
        <el-form :inline="true" :model="q">
          <el-form-item label="计划分类">
            ...
            >
              ...
              <el-option label="天窗管理计划" value="天窗管理计划"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="计划状态" style="margin-left: 15px">
            <el-select v-model="q.state" placeholder="请选择状态" value="已发布" size="small">
      ...
      >
        ...
        >
          ...
          <el-form-item label="计划分类" prop="cate_id">
            <el-select
      ...
      <!-- 查看文章详情的对话框 -->
      <!-- 查看文章详情的对话框 -->
      <el-dialog title="计划详情" :visible.sync="detailVisible" width="80%">
  ...
  methods: {
    ...
    pubArticleFn (state) {
      ...
      this.$refs.pubFormRef.validate(async (valid) => {
        if (!valid) return this.$message.error('请完善计划信息！')
        // 3. 判断是否提供了文章封面
        ...
        console.log(res)
        if (res.code !== 0) return this.$message.error('发布计划失败！')
        this.$message.success('发布计划成功！')
    ...
    async initArtListFn () {
      ...
      if (res.code !== 0) return this.$message.error('获取计划列表失败!')
    ...
    async showDetailFn (id) {
      ...
      if (res.code !== 0) return this.$message.error('获取计划详情失败!')
